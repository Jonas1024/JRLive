...
#import "JRLiveSession.h"
#import "JRVideoCapture.h"
#import "JRAudioCapture.h"
#import "JRHardwareVideoEncoder.h"
#import "JRHardwareAudioEncoder.h"
#import "JRStreamRTMPSocket.h"
#import "JRLiveStreamInfo.h"
#import "JRGPUImageBeautyFilter.h"

@interface JRLiveSession ()<JRAudioCaptureDelegate, JRVideoCaptureDelegate, JRAudioEncodingDelegate, JRVideoEncodingDelegate, JRStreamSocketDelegate>
...
/// 声音采集
@property (nonatomic, strong) JRAudioCapture *audioCaptureSource;
/// 视频采集
@property (nonatomic, strong) JRVideoCapture *videoCaptureSource;
/// 音频编码
@property (nonatomic, strong) id<JRAudioEncoding> audioEncoder;
/// 视频编码
@property (nonatomic, strong) id<JRVideoEncoding> videoEncoder;
/// 上传
@property (nonatomic, strong) id<JRStreamSocket> socket;


#pragma mark -- 内部标识
/// 调试信息
@property (nonatomic, strong) JRLiveDebug *debugInfo;
/// 流信息
@property (nonatomic, strong) JRLiveStreamInfo *streamInfo;
/// 是否开始上传
@property (nonatomic, assign) BOOL uploading;
/// 当前状态
@property (nonatomic, assign, readwrite) JRLiveState state;
/// 当前直播type
@property (nonatomic, assign, readwrite) JRLiveCaptureTypeMask captureType;
/// 时间戳锁
...
@implementation JRLiveSession

- (instancetype)initWithAudioConfiguration:(nullable JRLiveAudioConfiguration *)audioConfiguration videoConfiguration:(nullable JRLiveVideoConfiguration *)videoConfiguration {
    return [self initWithAudioConfiguration:audioConfiguration videoConfiguration:videoConfiguration captureType:JRLiveCaptureDefaultMask];
}

- (nullable instancetype)initWithAudioConfiguration:(nullable JRLiveAudioConfiguration *)audioConfiguration videoConfiguration:(nullable JRLiveVideoConfiguration *)videoConfiguration captureType:(JRLiveCaptureTypeMask)captureType
{
    if((captureType & JRLiveCaptureMaskAudio || captureType & JRLiveInputMaskAudio) && !audioConfiguration) @throw [NSException exceptionWithName:@"JRLiveSession init error" reason:@"audioConfiguration is nil " userInfo:nil];
    if((captureType & JRLiveCaptureMaskVideo || captureType & JRLiveInputMaskVideo) && !videoConfiguration) @throw [NSException exceptionWithName:@"JRLiveSession init error" reason:@"videoConfiguration is nil " userInfo:nil];
    if (self = [super init]) {
...
- (void)startLive:(JRLiveStreamInfo *)streamInfo {
...
- (void)pushVideo:(nullable CVPixelBufferRef)pixelBuffer{
    if(self.captureType & JRLiveInputMaskVideo){
        if (self.uploading) [self.videoEncoder encodeVideoData:pixelBuffer timeStamp:NOW];
    }
}

- (void)pushAudio:(nullable NSData*)audioData{
    if(self.captureType & JRLiveInputMaskAudio){
...
- (void)pushSendBuffer:(JRFrame*)frame{
...
- (void)captureOutput:(nullable JRVideoCapture *)capture pixelBuffer:(nullable CVPixelBufferRef)pixelBuffer {
    if (self.uploading) [self.videoEncoder encodeVideoData:pixelBuffer timeStamp:NOW];
}
...
- (void)socketStatus:(nullable id<JRStreamSocket>)socket status:(JRLiveState)status {
    ...
    } else if(status == JRLiveStop || status == JRLiveError){
        self.uploading = NO;
    }
...
- (void)socketBufferStatus:(nullable id<JRStreamSocket>)socket status:(JRLiveBuffferState)status {
    if((self.captureType & JRLiveCaptureMaskVideo || self.captureType & JRLiveInputMaskVideo) && self.adaptiveBitrate){
        NSUInteger videoBitRate = [self.videoEncoder videoBitRate];
        if (status == JRLiveBuffferDecline) {
...
- (JRAudioCapture *)audioCaptureSource {
    if (!_audioCaptureSource) {
        if(self.captureType & JRLiveCaptureMaskAudio){
            _audioCaptureSource = [[JRAudioCapture alloc] initWithAudioConfiguration:_audioConfiguration];
...
- (JRVideoCapture *)videoCaptureSource {
    if (!_videoCaptureSource) {
        if(self.captureType & JRLiveCaptureMaskVideo){
            _videoCaptureSource = [[JRVideoCapture alloc] initWithVideoConfiguration:_videoConfiguration];
...
- (id<JRAudioEncoding>)audioEncoder {
    if (!_audioEncoder) {
        _audioEncoder = [[JRHardwareAudioEncoder alloc] initWithAudioStreamConfiguration:_audioConfiguration];
        [_audioEncoder setDelegate:self];
    }
    return _audioEncoder;
}

- (id<JRVideoEncoding>)videoEncoder {
    if (!_videoEncoder) {
        _videoEncoder = [[JRHardwareVideoEncoder alloc] initWithVideoStreamConfiguration:_videoConfiguration];
        [_videoEncoder setDelegate:self];
    }
    return _videoEncoder;
}

- (id<JRStreamSocket>)socket {
    if (!_socket) {
        _socket = [[JRStreamRTMPSocket alloc] initWithStream:self.streamInfo reconnectInterval:self.reconnectInterval reconnectCount:self.reconnectCount];
        [_socket setDelegate:self];
    }
    return _socket;
}

- (JRLiveStreamInfo *)streamInfo {
    if (!_streamInfo) {
        _streamInfo = [[JRLiveStreamInfo alloc] init];
    }
    return _streamInfo;
}
...
- (BOOL)AVAlignment{
    if((self.captureType & JRLiveCaptureMaskAudio || self.captureType & JRLiveInputMaskAudio) &&
       (self.captureType & JRLiveCaptureMaskVideo || self.captureType & JRLiveInputMaskVideo)
       ){
