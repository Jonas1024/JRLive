...
@interface JRStreamingBuffer (){
    dispatch_semaphore_t _lock;
}

@property (nonatomic, strong) NSMutableArray <JRFrame *> *sortList;
@property (nonatomic, strong, readwrite) NSMutableArray <JRFrame *> *list;
...
@implementation JRStreamingBuffer
...
- (void)appendObject:(JRFrame *)frame
{
    if (!frame) return;
    if (!_startTimer) {
        _startTimer = YES;
    ...
    if (self.sortList.count < defaultSortBufferMaxCount) {
        ...
        JRFrame *firstFrame = [self.sortList jrPopFirstObject];
...
- (JRFrame *)popFirstObject {
    dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);
    JRFrame *firstFrame = [self.list jrPopFirstObject];
    dispatch_semaphore_signal(_lock);
    return firstFrame;
}
...
- (NSArray *)expirePFrames
{
    ...
    for (NSInteger index = 0; index < self.list.count; index++) {
        JRFrame *frame = [self.list objectAtIndex:index];
        if ([frame isKindOfClass:[JRVideoFrame class]]) {
            JRVideoFrame *videoframe = (JRVideoFrame *)frame;
...
- (NSArray *)expireIFrames
{
    ...
    for (NSInteger index = 0; index < self.list.count; index++) {
        JRFrame *frame = [self.list objectAtIndex:index];
        if ([frame isKindOfClass:[JRVideoFrame class]] && ((JRVideoFrame *)frame).isKeyFrame) {
...
- (JRLiveBuffferState)currentBufferState {
    ...
    if (increaseCount >= self.callBackInterval) {
        return JRLiveBuffferIncrease;
    }

    if (decreaseCount >= self.callBackInterval) {
        return JRLiveBuffferDecline;
    }
    
    return JRLiveBuffferUnknown;
}
...
- (void)tick
{
    ...
    if (self.currentInterval >= self.callBackInterval) {
        JRLiveBuffferState state = [self currentBufferState];
        if (state == JRLiveBuffferIncrease) {
            if (self.delegate && [self.delegate respondsToSelector:@selector(streamingBuffer:bufferState:)]) {
                [self.delegate streamingBuffer:self bufferState:JRLiveBuffferIncrease];
            }
        } else if (state == JRLiveBuffferDecline) {
            if (self.delegate && [self.delegate respondsToSelector:@selector(streamingBuffer:bufferState:)]) {
                [self.delegate streamingBuffer:self bufferState:JRLiveBuffferDecline];
            }
        }
...
NSInteger frameDataCompare(id obj1, id obj2, void *context){
    JRFrame *frame1 = (JRFrame *)obj1;
    JRFrame *frame2 = (JRFrame *)obj2;
