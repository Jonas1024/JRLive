...
@interface JRStreamRTMPSocket ()<JRStreamingBufferDelegate>
{
    PILI_RTMP *_rtmp;
}
@property (nonatomic, weak) id<JRStreamSocketDelegate> delegate;
@property (nonatomic, strong) JRLiveStreamInfo *stream;
@property (nonatomic, strong) JRStreamingBuffer *buffer;
...
@implementation JRStreamRTMPSocket
...
- (void)_start
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]) {
        [self.delegate socketStatus:self status:JRLivePending];
    }
...
- (void)_stop {
    if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]) {
        [self.delegate socketStatus:self status:JRLiveStop];
    }
    if (_rtmp != NULL) {
        PILI_RTMP_Close(_rtmp, &_error);
        PILI_RTMP_Free(_rtmp);
...
- (NSInteger)RTMP264_Connect:(char *)push_url
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]) {
        [self.delegate socketStatus:self status:JRLiveStart];
    }
...
- (void)sendVideoHeader:(JRVideoFrame *)videoFrame
{
...
- (void)sendVideo:(JRVideoFrame *)frame
{
    ///h264 https://zhuanlan.zhihu.com/p/83454344
    NSInteger i = 0;
    ...
    body[i++] = 0x00;
    body[i++] = 0x00;
    body[i++] = (frame.data.length >> 24) & 0xff;   //大端
...
- (void)sendAudioHeader:(JRAudioFrame *)audioFrame
{
...
- (void)reconnect
{
    dispatch_async(self.rtmpSendQueue, ^{
        if (self.retryTimes4netWorkBreaken++ < self.reconnectCount && !self.isReconnecting) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]) {
                [self.delegate socketStatus:self status:JRLiveError];
            }
            if (self.delegate && [self.delegate respondsToSelector:@selector(socketDidError:errorCode:)]) {
                [self.delegate socketDidError:self errorCode:JRLiveSocketError_ReConnectTimeOut];
            }
        }
    });
}
...
- (void)_reconnect{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:status:)]) {
        [self.delegate socketStatus:self status:JRLiveRefresh];
    }
...
- (void)sendFrame
{
    ...
    dispatch_async(self.rtmpSendQueue, ^{
        if (!_self.isSending && _self.buffer.list.count > 0) {
            ...
            // 调用发送接口
            JRFrame *frame = [_self.buffer popFirstObject];
            if ([frame isKindOfClass:[JRVideoFrame class]]) {
                if (!_self.sendVideoHead) {
                    _self.sendVideoHead = YES;
                    if(!((JRVideoFrame*)frame).sps || !((JRVideoFrame*)frame).pps){
                        _self.isSending = NO;
                        return;
                    }
                    [_self sendVideoHeader:(JRVideoFrame *)frame];
                } else {
                    [_self sendVideo:(JRVideoFrame *)frame];
                }
            } else {
                if (!_self.sendAudioHead) {
                    _self.sendAudioHead = YES;
                    if(!((JRAudioFrame*)frame).audioInfo){
                        _self.isSending = NO;
                        return;
                    }
                    [_self sendAudioHeader:(JRAudioFrame *)frame];
                } else {
            ...
            if (_self.debugInfo.elapsedMilli < 1000) {
                ...
                if ([frame isKindOfClass:[JRAudioFrame class]]) {
...
{
    JRStreamRTMPSocket *socket = (__bridge JRStreamRTMPSocket *)userData;
    if (error->code < 0) {
        [socket reconnect];
    }
}
...
- (JRStreamingBuffer *)buffer {
    if (!_buffer) {
        _buffer = [[JRStreamingBuffer alloc] init];
...
- (JRLiveDebug *)debugInfo {
    if (!_debugInfo) {
        _debugInfo = [[JRLiveDebug alloc] init];
    }
    return _debugInfo;
}
...
- (void)setDelegate:(id<JRStreamSocketDelegate>)delegate {
    _delegate = delegate;
}
