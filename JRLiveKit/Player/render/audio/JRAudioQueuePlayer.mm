...
#define kJRAudioPCMFramesPerPacket 1
#define kJRAudioPCMBitsPerChannel  16
...
typedef struct JRAudioInfo *JRAudioInfoRef;

static JRAudioInfoRef m_audioInfo;

@interface JRAudioQueuePlayer ()
...
@implementation JRAudioQueuePlayer
...
static void PlayAudioDataCallback(void * aqData,AudioQueueRef inAQ , AudioQueueBufferRef inBuffer) {
    JRAudioQueuePlayer *instance = (__bridge JRAudioQueuePlayer *)aqData;
    if(instance == NULL){
        return;
    }
...
static void AudioQueuePlayerPropertyListenerProc  (void *              inUserData,
    ...
    JRAudioQueuePlayer * instance = (__bridge JRAudioQueuePlayer *)inUserData;
    UInt32 isRunning = 0;
...
+ (void)initialize
{
    int size = sizeof(JRAudioInfo);
    m_audioInfo = (JRAudioInfoRef)malloc(size);
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self->_audioBufferQueue   = new JRCustomQueueProcess();
    }
    return self;
}
...
{
    memcpy(&m_audioInfo->mDataFormat, audioFormat, sizeof(JRAudioInfo));
...
#pragma mark - Private
- (BOOL)configureAudioPlayerWithAudioInfo:(JRAudioInfoRef)audioInfo playCallback:(AudioQueueOutputCallback)playCallback listenerCallback:(AudioQueuePropertyListenerProc)listenerCallback
{
...
- (void)receiveAudioDataWithAudioQueueBuffer:(AudioQueueBufferRef)inBuffer audioInfo:(JRAudioInfoRef)audioInfo audioBufferQueue:(JRCustomQueueProcess *)audioBufferQueue
{
    JRCustomQueueNode *node = audioBufferQueue->DeQueue(audioBufferQueue->m_work_queue);
...
- (BOOL)startAudioPlayerWithAudioInfo:(JRAudioInfoRef)audioInfo
{
    for (int i = 0; i != kNumberBuffers; i++) {
...
- (BOOL)pauseAudioPlayerWithAudioInfo:(JRAudioInfoRef)audioInfo {
...
- (BOOL)resumeAudioPlayerWithAudioInfo:(JRAudioInfoRef)audioInfo {
...
-(BOOL)stopAudioQueueRecorderWithAudioInfo:(JRAudioInfoRef)audioInfo {
    if (audioInfo->mQueue) {
...
- (BOOL)freeAudioQueueRecorderWithAudioInfo:(JRAudioInfoRef)audioInfo
{
    if (audioInfo->mQueue) {
        for (int i = 0; i < kNumberBuffers; i++) {
...
-(AudioStreamBasicDescription)getAudioFormatWithFormatID:(UInt32)formatID sampleRate:(Float64)sampleRate channelCount:(UInt32)channelCount {
    ...
    if (formatID == kAudioFormatLinearPCM) {
        ...
        dataFormat.mBitsPerChannel  = kJRAudioPCMBitsPerChannel;
        dataFormat.mBytesPerPacket  = dataFormat.mBytesPerFrame = (dataFormat.mBitsPerChannel / 8) * dataFormat.mChannelsPerFrame;
        dataFormat.mFramesPerPacket = kJRAudioPCMFramesPerPacket;
