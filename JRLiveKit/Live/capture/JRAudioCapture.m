...
NSString *const JRAudioComponentFailedToCreateNotification = @"JRAudioComponentFailedToCreateNotification";

@interface JRAudioCapture ()

@property (nonatomic, assign) AudioComponentInstance componetInstance;
@property (nonatomic, assign) AudioComponent component;
@property (nonatomic, strong) dispatch_queue_t taskQueue;
@property (nonatomic, assign) BOOL isRunning;
@property (nonatomic, strong,nullable) JRLiveAudioConfiguration *configuration;


@end

@implementation JRAudioCapture

- (instancetype)initWithAudioConfiguration:(JRLiveAudioConfiguration *)configuration{
    if(self = [super init]){
        _configuration = configuration;
        self.isRunning = NO;
        self.taskQueue = dispatch_queue_create("com.dajiufan.audioCapture.Queue", NULL);
...
- (void)handleAudioComponentCreationFailure {
    dispatch_async(dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:JRAudioComponentFailedToCreateNotification object:nil];
    });
}
...
static OSStatus handleInputBuffer(void *inRefCon,
    ...
    @autoreleasepool {
        JRAudioCapture *source = (__bridge JRAudioCapture *)(inRefCon);
        if (!source) {
            return  -1;
        }
        AudioBuffer buffer;
