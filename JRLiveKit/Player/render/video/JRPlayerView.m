...
typedef enum : NSUInteger {
    JRPixelBufferTypeNone = 0,
    JRPixelBufferTypeNV12,
    JRPixelBufferTypeRGB,
} JRPixelBufferType;
...
GLfloat kPreViewColorConversion601FullRange[] = {
    1.0,    1.0,    1.0,
    0.0,    -0.343, 1.765,
    1.4,    -0.711, 0.0,
};
...
@property (nonatomic, assign) JRPixelBufferType bufferType;
@property (nonatomic, assign) JRPixelBufferType lastBufferType;
...
- (void)initPreview {
    ...
    self.screenWidth = 0;
    self.bufferType = JRPixelBufferTypeNV12;
    self.lastBufferType = JRPixelBufferTypeNone;
    _preferredConversion = kPreViewColorConversion601FullRange;
...
- (void)displayPixelBuffer:(CVPixelBufferRef)pixelBuffer
         ...
         frameBufferHandle:(GLuint)frameBufferHandle
               nv12Program:(GLuint)nv12Program
                rgbProgram:(GLuint)rgbProgram
       preferredConversion:(const GLfloat *)preferredConversion
displayInputTextureUniform:(GLuint)displayInputTextureUniform
         colorBufferHandle:(GLuint)colorBufferHandle
{
    if (pixelBuffer == NULL) {
        return;
    }
    ...
    if (bufferType == JRPixelBufferTypeNV12) {
        // Y
    ...
    } else if (bufferType == JRPixelBufferTypeRGB) {
        // RGB
        glActiveTexture(GL_TEXTURE0);
        error = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault,
                                                             videoTextureCache,
...
- (EAGLContext *)createOpenGLContextWithWidth:(int *)width
                                       height:(int *)height
                            videoTextureCache:(CVOpenGLESTextureCacheRef *)videoTextureCache
                            colorBufferHandle:(GLuint *)colorBufferHandle
                            frameBufferHandle:(GLuint *)frameBufferHandle
{
    ...
    [self loadShaderWithBufferType:JRPixelBufferTypeNV12];
    [self loadShaderWithBufferType:JRPixelBufferTypeRGB];
...
- (void)setupBuffersWithContext:(EAGLContext *)context
                          width:(int *)width
                         height:(int *)height
              colorBufferHandle:(GLuint *)colorBufferHandle
              frameBufferHandle:(GLuint *)frameBufferHandle
...
- (void)loadShaderWithBufferType:(JRPixelBufferType)type
{
    ...
    if (type == JRPixelBufferTypeNV12) {
        shaderName = @"JRPreviewNV12Shader";
        _nv12Program = program;
    } else if (type == JRPixelBufferTypeRGB) {
        shaderName = @"JRPreviewRGBShader";
        _rgbProgram = program;
    }
